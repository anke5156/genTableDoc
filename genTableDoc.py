...
class GenTableSchemal(object):
    ...
    def _sqls(self, id):
        ...
        if id == 0:
            ...
            return f"""
                    ...
                    and b.table_name = '{self.tab_en}' -- 你要查的表
    ...
    def _writeExcel(self, excelName, data):
        ...
        for i, tb in enumerate(self.tabs):
            for self.tab_en, self.tab_cn in tb.items():
                pass
            self.sheetName = f'tbl{i + 1}'
            db_read = self.db.read(self._sqls(1), as_dict=False)
            sheet = writebook.add_sheet(self.sheetName)

            sheet.write_merge(0, 0, 0, 5, self.tab_en)
            sheet.write_merge(1, 1, 0, 5, self.tab_cn)  # 开始行,结束行, 开始列, 结束列
            link = 'HYPERLINK("#表列表!B3";"返回")'
            sheet.write_merge(2, 2, 0, 5, xlwt.Formula(link))

            title = ['表名', '字段序号', '字段', '类型', '主键', '字段描述']
            # 表头
            for j, cell in enumerate(title):
                style.font = fontT
                style.alignment = alignment1
    ...
    def queryDb(self):
        ...
        :return:
        """
        data = [['数据库', '表名', '表注释', '类型', '数据量', '创建时间', '更新时间', '表结构']]
        read = self.db.read(self._sqls(0), as_dict=False)
        for lin in read.get_all():
            self.tabs.append({lin[1]: lin[2]})
            data.append(lin)
        print(self.tabs)
        self._writeExcel('tableSche.xlsx', data)
